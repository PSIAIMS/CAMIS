---
title: "How to Chose an R Package"
---

When you are looking at how to do stats methods in R and you don't already know the package that does the method, it can be difficult to figure out the "best" package. Sometimes when you google a method you just get the package with wording that matches closest to what you typed into google, rather than the package the is "best" for the method. You don't want to waste time looking deeply into a package only to find there is a more common and robust R package out there for the same method. So before you go down a rabbit hole on one package, it is worth checking a few packages. But, how do you "check" a package. This blog post will go through a worked example of comparing packages and a checklist of things to look at when you are comparing packages in the future.
...
For this, we are going to look at the Wilcoxon Rank-Sum test and the associated Hodges-Lehmann confidence interval. After googling a bit, I found three different packages that do a Wilcoxon Rank-Sum p-value and Hodges-Lehmann CI, {stats} (part of base R), {pairwiseCI}, {coin}.
...
::: callout-tip
Often times, differences between package and software show up when there are ties, missing data, and/or extreme values, so it is good to try to include these in the dataset you are using to compare.
:::
...
-   Is the author known in the field?
    ...
    Personally, I don't know this author, but it looks like he does work at a biostatistics department at a university, so that is a really good sign. When you are looking at the author you don't need to go super in depth, but if you don't know who they are it can be good to check the qualifications.
    ...
    -   Is there more than one author?
        ...
        Yes, it looks like there are two authors here. This can be good the check because it can mean the burden of maintaining the package is shared and the documentation has potentially been peer reviewed a bit.
...
-   Does one of the packages have better documentation?
    ...
    -   Are there references to external peer reviewed papers?
        ...
        On the index there are no references, but ther might be some references on the function level. Really it just means they haven't published a paper about this package.
