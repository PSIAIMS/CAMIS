...
-   [`{r} R.Version()$version.string`](https://www.r-project.org/)
    -   If possible, utilize `{r} glue::glue("R ", R.Version()$major,".", R.Version()$minor)`, as this is the version utilized in the repository.
...
The CAMIS repository utilizes [`renv`](https://rstudio.github.io/renv/articles/renv.html), which is an R package that helps create reproducible environments. Assuming you downloaded [`{r} glue::glue("R ", R.Version()$major,".", R.Version()$minor)`](https://www.r-project.org/), which is the version of R associated with this `renv` project, everything should be relatively straightforward.

If you have R ≥ 4.1, you generally shouldn't have issues, but `renv` will alert you that `{r} glue::glue("R ", R.Version()$major,".", R.Version()$minor)` was used to install the packages and that `{r} glue::glue("R ", R.Version()$major,".", R.Version()$minor)` is different than the R version you are using. As more R packages are added/updated in CAMIS `renv`, there may arise issues if your R version differs greatly from `{r} glue::glue("R ", R.Version()$major,".", R.Version()$minor)`. This may require you to install a version of R closer to `{r} glue::glue("R ", R.Version()$major,".", R.Version()$minor)`, so please work with your appropriate technology professional to upgrade R.
...
#| echo: false
#| results: asis
renv_version <- packageDescription("renv")$Version
cat("```r\n")
cat(glue::glue('install.packages("renv@{renv_version}")'), "\n")
cat("```")
```
...
cat(
  glue::glue(
    "- Project '(path/to/CAMIS/project)' loaded. [renv {renv_version}]"
  ),
  "\n"
)
cat("```")
...
The final step is to add a reviewer. Please add  [\@DrLynTaylor](https://github.com/DrLynTaylor), [\@statasaurus](https://github.com/statasaurus) and [\@chizapoth](https://github.com/chizapoth). For more details about making pull requests see [create a pull request](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/proposing-changes-to-your-work-with-pull-requests/creating-a-pull-request).
