...
Several asymptotic methods offer a continuity adjustment, which aims to emulate 'exact' methods and achieve almost conservative coverage, by for example adding a quantity of the order $0.5 / N$ to the formula. Generally this is not a successful modification to the Wald method, but can produce good results for the score methods. It has been suggested that an adjustment of smaller magnitude such as $0.25 / N$[@laud2017, Appendix S2] (or any other selected value on a 'sliding scale' from 0 to 0.5) could be used to achieve coverage that is at least $(1-\alpha)$ nearly all of the time.
...
**The [{cardx} package](https://insightsengineering.github.io/cardx)** is an extension of the {cards} package, providing additional functions to create Analysis Results Data Objects (ARDs). It was developed as part of {NEST} and pharmaverse. This package requires the binary endpoint to be a logical (TRUE/FALSE) vector or a numeric/integer coded as (0, 1) with 1 (TRUE) being the success you want to calculate the confidence interval for.
...
**The {ratesci} package** contains functions to produce a variety of CIs for a single binomial proportion or Poisson event rate, and all contrasts of independent proportions/rates or paired proportions. It has a particular emphasis on classes of methods that can be applied across different analysis types, in particular the Asymptotic Score methods (with or without skewness correction) which come with naturally consistent hypothesis tests (for association or non-inferiority), and also caters for analysis of stratified datasets. The full range of MOVER (or 'square-and-add') methods is also included.
