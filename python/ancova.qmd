...
```
There are different types of ancova computations. The [statsmodels.stats.ancova.ancova_lm](https://www.statsmodels.org/stable/generated/statsmodels.stats.ancova.ancova_lm.html) function allows the types 1, 2 and 3. 

```{python, eval=TRUE, echo=TRUE}
from statsmodels.formula.api import ols
from statsmodels.stats.ancova import ancova_lm
from tabulate import tabulate

# Center the predictor for Type III ancova
df['pre_centered'] = df['pre'] - df['pre'].mean()

# # Fit the model for types I and II
model = ols('post ~ C(drug) + pre', data=df).fit()

# # Perform ancova for types I and II
ancova_table_type_1 = ancova_lm(model, typ=1)
ancova_table_type_2 = ancova_lm(model, typ=2)

# # Fit the model for Type III ancova with centered predictors
model_type_3 = ols('post ~ C(drug) + pre_centered', data=df).fit()
ancova_table_type_3 = ancova_lm(model_type_3, typ=3)
