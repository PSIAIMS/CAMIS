...
title: "SAS vs R Non-parametric Analysis - Friedman test"
...
Save R-generated data for SAS to use.

```{r}
write_csv(df_bp, paste0(getwd(),"/data_bp.csv"))
```

SAS will import generated dataset from the corresponding location.

```{=html}
<pre><code>libname data '\\Client\Y$\Gitlab\CAMIS';

proc import datafile="%sysfunc(pathname(data))\data_bp.csv" out=data_bp dbms=csv replace;
    getnames=yes;
run;
</code></pre>
```
...
```{=html}
<pre><code>proc freq data=data_bp;
