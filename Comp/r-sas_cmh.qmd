...
```{r, echo = F}
sas_odds <- tribble(
  ~Scenario, ~OR, ~LCL, ~UCL, ~prob, ~lang, 
  1 , 0.9550959, 0.4865116, 1.874998, NA_real_, "SAS"
)

freq_dat <- data |> 
  filter(TRTP != "Xanomeline High Dose") |> 
  group_by(TRTP , SEX , AGEGR1) |> 
  dplyr::summarise(Freq = n_distinct(USUBJID)) 
freq_tabs <- xtabs(Freq ~ TRTP + SEX + AGEGR1, data = freq_dat)
foo <- mantelhaen.test(freq_tabs)
...
```{r, echo = F}
# readRDS(('cmh/Docu/interim_update','table3.RDS'))
```
