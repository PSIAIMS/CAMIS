...
The main difference between the two methods is that LF method requires specification of accrual duration as well as study duration, while KM method calculates study duration iteratively given accrual rates and accrual duration. In general, these two methods produce similar, but not identical results.

Both LF and KM methods are implemented in gsDesign, while KT method is implemented in EAST and rpact. gsDesign2 uses LF method along with average hazard ratio (AHR) method (Schemper, Wakounig, and Heinze, 2009, Yung and Liu 2020) for non-proportional hazard assumption.

One additional computational difference to note for EAST vs gsDesign/gsDesign2 is the usage of different log hazard ratio variance assumptions. By default, EAST uses the variance under the null hypothesis and provides an option for using the variance under the alternative hypothesis. gsDesign, on the other hand, is using both of these variances. gsDesign2 has `info_scale` argument in `gsDesign::gs_power_ahr()`, `gsDesign::gs_design_ahr()`, which could be set to null or alternative hypothesis for variance assumption or the combination of variances.

Below we provide an example of reproducing EAST results from [this vignette](https://psiaims.github.io/CAMIS/East/gsd-tte.html) using gsDesign/gsDesign2/rpact.

## Design example

We assume that a GSD is utilized for progression-free survival (PFS) endpoint. It will be tested at one interim analysis (IA) for both efficacy and non-binding futility and then at final analysis (FA). O'Brein-Fleming spending function will be used for efficacy testing and Hwang-Shih-Decani spending function with $\gamma = -10$ will be used for futility.
...
# using the number of events that was calculated by EAST
timing_pfs <- c(176/235, 1)
...
We assume that EAST was initially used to calculate the target number of events and the total sample size and we will use gsDesign/gsDesign2/rpact to reproduce these.

Note that, in EAST the number of target events is reported as an integer, however, gsDesign/gsDesign2 provide non-integer values which match *exactly* the specified information fraction. Therefore, in order to reproduce EAST results, we will use the number of events that was calculated in EAST for `timing` argument in `gsDesign::gsSurv()`: 176 and 235 PFS events for IA and FA respectively (please see the `timing_pfs` object in the code above).
...
-   The comparison between EAST and gsDesign/gsDesign/rpact results is presented below using absolute difference in efficacy/futility boundaries and crossing probabilities up to 4 decimals. Non-zero values will be highlighted.
-   Note that, in gsDesign/gsDesign efficacy/futility bounds are referred to as “upper”/"lower" bounds respectively, while in EAST these appear to be "lower"/"upper" bounds respectively. For the comparison purposes, we will assume that efficacy/futility bounds are “upper”/"lower" bounds respectively.
...
pfs_gsDesign |>
  ...
  dplyr::bind_cols(pfs_east |>
                     dplyr::select(value, analysis, eff_east = eff_125, fut_east = fut_125) |>
                     dplyr::mutate(
  ...
  dplyr::select(Analysis = analysis, Value = value, diff_eff_gsDesign, diff_fut_gsDesign) |>
  flextable::flextable() |>
  flextable::autofit() |>
  ...
  flextable::bg(~ round(abs(diff_eff_gsDesign), digit_comp) != 0,  ~ diff_eff_gsDesign, bg = '#6ECEB2') |>
  flextable::bg(~ round(abs(diff_fut_gsDesign), digit_comp) != 0,  ~ diff_fut_gsDesign, bg = '#6ECEB2') 
```
...
pfs_gsDesign2 |>
  ...
  flextable::hline(i = c(5, 10)) |>
  flextable::colformat_double(j = c(1, 2), digits = digit_comp) |>
  flextable::bg(~ round(abs(diff_eff_gsDesign), digit_comp) != 0,  ~ diff_eff_gsDesign, bg = '#6ECEB2') |>
  flextable::bg(~ round(abs(diff_fut_gsDesign), digit_comp) != 0,  ~ diff_fut_gsDesign, bg = '#6ECEB2') 
```
...
Note: the `dropoutRate1`, `dropoutRate2` arguments in the `getSampleSizeSurvival()` refer to the % of drop-outs by the `dropoutTime`, while the `eta` argument in `gsDesign::gsSurv()` and the `dropout_rate` value in the `fail_rate` argument in `gsDesign2::gs_design_ahr()` refer to the annual drop-out rate parameter under the exponential distribution. In our example, if $X$ is a drop-out time and $X \sim Exponential (\lambda)$, we assume that by month 12 the drop-out rate was 20%, which implies: $P(X\le12) = 1 - e^{-12\lambda} = 0.2 \Rightarrow \lambda = 0.019$. Due to the above differences the value $\lambda = 0.019$ was used in the gsDesign and gsDesign2 example, while 0.2 was used in the rpact example.
