...
Comparison of results between SAS vs R for different applications of logistic regression; where possible we try to ensure the same statistical method or algorithm is specified. However, there are some underlying differences between the algorithms in SAS vs R that cannot be (at least not easily) "tweaked". The document also provides a discussion about things to watch out for when comparing logistic regression results in R versus SAS, potential mismatches in the results, and what might have caused these differences.
...
Below are summary of findings from a numerical comparison using example data, where possible we specify the same algorithm in R and SAS (see section [Numerical Comparisons](#sec-num-comp) for details).
...
- Estimate and 95% CI for conditional odds ratio matched at 0.001 level;\
- Standard error estimates for logistic regression coefficients are not the same (e.g., 0.6331 for Treatment Group in R vs 0.6542 in SAS), and as a result the p-values are not the same (0.0153 in R vs 0.0229 in SAS);\
...
In the following sections the parameterisation of each implementation will be compared in tabular fashion followed by a numerical comparison using example data.
...
### Logistic regression and firth logistic regression

We will use the `lung` dataset provided with {survival} R package. Initial data preparation involves generating a new binary outcome based on weight change.
...
# write.csv(lung2, file = "data/lung.csv", na = ".")
```

### g-computation

We will use the `trial01` dataset provided with {beeca} R package. Initial data preparation involves setting the treatment indicator as a categorical variable and removing any incomplete cases.
...
## remove missing data i.e complete cases analysis
trial01 <- trial01 |>
...
-   R `stats::glm` function.

-   SAS `PROC LOGISTIC` procedure (with and without firth option)

Every effort is made to ensure that the R code employs estimation methods/ optimization algorithms/ other components that closely match (as much as possible) those used in the SAS code. This is done to facilitate a comparison of coefficient estimates, conditional odds ratio, and p-values between the two software. 

### `glm` in R

The model coefficients summary is shown as below.
...
Conditional odds ratio is calculated by taking the exponential of the model parameters.

Note, function `confint.default` gives the Wald confidence limits, which is the default option in SAS `PROC LOGISTIC` procedure; whereas `confint` gives the profile-likelihood limits.
...
Below is screenshot of output tables summarizing coefficient estimates
...
Exact match (at 0.001 level) can be obtained using `glm` in R vs `PROC LOGISTIC` procedure (without Firth option) in SAS, for coefficient estimates, conditional odds ratio, and for p-value.
:::
...
-   The default fitting method is "Newton-Raphson" in `logistf` function, while the default fitting method for `PROC LOGISTIC` procedure in SAS is Fisher's scoring method (equivalent to "Iteratively reweighted least squares (IRLS)" method as reported in this [documentation](https://support.sas.com/documentation/cdl/en/statug/63347/HTML/default/viewer.htm#statug_logistic_sect033.htm)). Therefore, for comparison with SAS output, the `control` argument is specified with fit = "IRLS". (However, the parameter estimates and the estimated covariance matrices are the same with "IRLS" vs "Newton-Raphson").
...
## Function comment below would give Wald CI and Wald tests results by adding `pl = FALSE` in logistf function
# logistf(..., pl = FALSE)
```

Conditional odds ratio is calculated by taking the exponential of the model parameters.

Note, function `confint` gives the profile-likelihood limits. Given the parameters from Firth's bias-reduced logistic regression is estimated using penalized maximum likelihood, `confint` function is used.

```{r}
exp(cbind(OR = coef(firth_mod), 
          confint(firth_mod)))
```

### `PROC LOGISTIC` in SAS (with firth option)

SAS code is presented as below.
...
We now use the SAS %margins macro to perform the Ge et al. method on `trial01`. Although the SAS code below does not execute, it illustrates how the same analysis should be configured using the macro:
...
Note: the methods correspond at the 6th decimal place. 

# Discussion

Below is a few things we should be aware of when comparing logistic regression results in R (glm or logistf function) vs SAS (proc logistic),

-   What is the likelihood optimization method:
...
-   Difference in convergence criteria: although both SAS and R allows options to modify the convergence criteria, the criteria does not seem to be exactly the same, which **could cause results mismatch in some scenarios**.
...
-   [GitHub repository for a SAS procedure about Firth logistic regression authored by the author of `logistf` R package](https://github.com/georgheinze/flicflac/tree/master/LogisticRegression), which was probably authored before the availability of Firth option in PROC LOGISTIC statement in SAS.
