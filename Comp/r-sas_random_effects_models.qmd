---
title: "R vs SAS Random Effect Models"
message: false
warning: false
echo: true
eval: false
...
::: callout-note
As fixed effect estimates are entirely determined by the estimated V matrix which is a function of the estimated variance parameters, the agreement of fixed effect estimates depends on the agreement of covariance parameter estimates.
:::
...
It is possible to change the optimisation method used by lme4. The lme4::allFit function can be used to check whether the different optimisation methods give the same results. 
...
## Degrees of freedom and Kenward-Rodger adjusted standard errors 

There is generally very close agreement between PROC MIXED and lme4/lmerTest for Satterthwaite degrees of freedom and Kenward-Rodger degrees of freedom and adjusted standard errors. Some differences may arise in the Kenward-Rodger results, particularly for small samples sizes, because of the information matrix which is used in the calculations. 

By default PROC MIXED uses the observed information matrix in the Kenward-Rodger calculations, whereas lmerTest uses the expected information matrix. It is possible to force PROC MIXED to use the expected information matrix by using the SCORING option. When SCORING = n is specified, the procedure will use Fisher scoring for the first n iterations. If the model converges when the Fisher scoring method is still being used, SAS will use the expected information to implement the Kenward-Roger method.

## Confidence intervals for model coefficients 

The confint function applied directly to the lme4 model in R will use the Wald method (method = "Wald") or profile likelihood (method = "profile") method to calculate confidence intervals for model coefficients. The Wald method uses the normal distribution (infinite degrees of freedom). If you would like to use the Kenward-Rodger or Satterthwaite degrees of freedom, either calculate the confidence interval manually using the degrees of freedom provided or construct a contrast using emmeans and then calculate the confidence interval for the contrast. 

## Conclusion 

SAS and R implement linear mixed models under the same statistical framework but solve the likelihood optimisation problem using different numerical representations. Fixed-effect estimates typically agree closely once model specification is aligned. Differences arise primarily in covariance parameters and convergence diagnostics due to differences in parameterisation, optimisation geometry, and diagnostic thresholds. These differences are intrinsic to numerical computation and should be interpreted accordingly in clinical trial analyses. 
