...
The following code performs an ANCOVA analysis using the **sasLM** package.  This package was written specifically to replicate
SAS statistics.  The console output is also organized in a manner that is similar to SAS.
...
```
Note that the LSMEANS statistics are produced using the `EMEAN = TRUE` option.  The `BETA = TRUE` option is equivalent
to the `SOLUTION` option in SAS. See the **sasLM** documentation for additional information.