...
To cut down on computation time, you may want to increase the number of parallel processes using the modern `mirai` backend.

```{r}
#| label: setup_parallel
library(mirai)
library(purrr)

# Set up parallel processing with 4 daemons (background processes)
mirai::daemons(4)
```

### Perform tipping point analysis

To enable a tipping point analysis within a single function, we create `perform_tipp_analysis()`. This custom function requires a stratified delta for `delta_control` and `delta_intervention`. Note that we now use `in_parallel()` from purrr to wrap our analysis function for parallel execution.
...
Now, let's apply this function to the antidepressant data using the new `in_parallel()` approach:

```{r}
#| label: MAR_v1_tipping_point_analysis
MAR_tipp_df1 <- delta_df1 |>
  mutate(
    results_list = purrr::map2(purrr::in_parallel(
      delta_control,
      delta_intervention,
      perform_tipp_analysis
    )),
    trt_7 = purrr::map_dbl(results_list, "trt_7"),
    pval_7 = purrr::map_dbl(results_list, "pval_7"),
    lci_7 = purrr::map_dbl(results_list, "lci_7"),
    uci_7 = purrr::map_dbl(results_list, "uci_7")
  ) |>
  select(-results_list)
```
...
MAR_tipp_df2 <- delta_df2 |>
  mutate(
    results_list = purrr::map2(
      ...
      delta_intervention,
      purrr::in_parallel(
        \(dc, di) perform_tipp_analysis(dc, di),
        perform_tipp_analysis = perform_tipp_analysis,
        imputeObj = imputeObj,
        ancova = ancova,
        pool = pool,
...
Don't forget to reset the daemons when you are done with parallel processing.
