...
The Blaker 'exact' CI matching SAS output (to at least 7 dps) may be obtained as follows from {ratesci}, or from the dedicated {BlakerCI} package. Output from `DescTools::BinomCI` and `PropCIs::blakerci()` is less precise, matching only to 4 decimal places.
