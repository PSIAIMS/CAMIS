...
The commonly used R packages for power and sample size calculations utilizing a GSD are: [gsDesing](https://keaven.github.io/gsDesign/) (also available as a [ShinyApp](https://rinpharma.shinyapps.io/gsdesign/)), [gsDesing2](https://merck.github.io/gsDesign2/), and [rpact](https://www.rpact.org/).

## Design assumptions

Using a toy example, we will assume that a primary objective of a phase III oncology trial is to compare a new therapy to a control in terms of progression-free survival (PFS) and overall survival (OS). Note that, in this example we have a family of primary endpoints, and not co-primary endpoints. A GSD will be utilized for each endpoint. PFS will be tested at one interim analysis (IA) for both efficacy and non-binding futility, while OS will be tested at two IAs for efficacy only. O'Brein-Fleming spending function will be used for efficacy testing and Hwang-Shih-Decani spending function with $\gamma = -10$ will be used for futility.
...
fail_rate_pfs <- tibble(
  stratum = "All", 
  duration = Inf, #could be set to Inf when proportional hazard is aasumed
