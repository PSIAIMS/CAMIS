...
Сommon aspect of criticism in machine learning pipelines concerns the point estimation of performance. Best model are selected by mean/median validation folds scores during model tuning with k-fold cross-validation without taking into account confidence intervals for the scores. This issue is easy to fix because existing procedure already provides the set of validation scores ready to do the math. So we came back to statistical inference, but in terms of model performance metrics rather than model itself. It's also possible to generate multiple resamples from single test dataset and obtain interval estimates for metrics of interest, although this approach is not widely adopted yet.

# Software infrastructure to make it work

Simple rule of thumb:

* if you prefer more functional, less object-oriented programming and/or relies on [tidyverse](https://www.tidyverse.org/) in your work, [tidymodels](https://www.tidymodels.org/) it the choice;
