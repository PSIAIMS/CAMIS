...
2.  **Inferential models** - models that can produce p-values, confidence intervals and/or Bayesian estimates when we use it to answer pre-formulated questions via some kind of hypothesis testing. It is worth noting that machine learning approaches like cross-validation are fully applicable for inferential model selection because of overfitting risk. Even if a researcher isn't interested in prediction quality for new samples, estimating model characteristics using out-of-fold samples using k-fold cross-validation is valuable and somewhat undervalued opportunity. See [Inferential Analysis](https://www.tmwr.org/inferential#inferential) and Chapter 11 in [Regression and Other Stories](https://avehtari.github.io/ROS-Examples/) for details.
