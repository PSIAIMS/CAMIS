...
Derive the confidence intervals for the separate proportions in each group, $p_1$ and $p_2$, using the Wilson Score Method equations as described [here](ci_for_prop.html). (Any other method may also be selected, for example Jeffreys equal-tailed intervals achieve improved symmetrical coverage, but this MOVER-Jeffreys variant is not available in SAS.)
